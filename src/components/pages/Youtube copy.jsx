...
const Youtube = () => {
  ...
  const search = async (query) => {
    await fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&type=video`
    )
  ...
  useEffect(() => {
    fetch(
      "https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=webstoryboy&key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&type=video"
    )
      .then((response) => response.json())
      .then((result) => setYoutube(result.items))
      .catch((error) => console.log("error", error));
